...
public class FishBossManager implements Listener {
	...
	private void spawnFallingBlocks(@NonNull Location loc) {
		...
		for (int i = 0; i < count; i++) {
			BlockData blockData = materials.get(RANDOM.nextInt(materials.size()));
			world.spawn(loc.clone().add(0, 0.3, 0), FallingBlock.class, fallingBlock -> {
				...
				fallingBlock.setVelocity(new Vector(Math.cos(angle) * speed, upward, Math.sin(angle) * speed));

				UUID fbId = fallingBlock.getUniqueId();
	...
	private <T> @Nullable T pickRandom(@NonNull List<T> list) {
		return list.isEmpty() ? null : list.get(RANDOM.nextInt(list.size()));
	}

	private @NonNull List<ItemStack> buildSecondaryLoot() {
	...
	private @NonNull String randomName() {
